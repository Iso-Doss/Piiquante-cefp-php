...
if (empty($erreurs)) {
    ...
    if ($resultat) {
        ...
        if (isset($utilisateur_connecter) && !empty($utilisateur_connecter) && is_array($utilisateur_connecter)) {
            ...
            header("location: index-authentification.php?page=connexion&erreur=Identifiant incorrect. Veuillez réessayer.");
        }
    } else {

        header("location: index-authentification.php?page=connexion&erreur=Oups!!! Une erreur s'est produite lors de la verification de l'utilisateur.");
    }
} else {
