...
if (empty($erreurs)) {
    ...
    if (is_object($bd)) {
        ...
        if ($resultat) {
            ...
            header("location: index-authentification.php?page=connexion&success=Inscription éffectué avec succèss. Veuillez vous connecté.");
        } else {

            header("location: index-authentification.php?page=inscription&erreur=Oupss!!! Une erreure s'est produite lors de l'enregistrement de l'utilisateur. Veuillez réessayer ou contacter l'admin du site.");
        }
    } else {

        header("location: index-authentification.php?page=inscription&erreur=" . $bd);
    }
} else {

    $_SESSION["erreurs_inscription"] = $erreurs;

    header("location: index-authentification.php?page=inscription");
}
